...
function simpleProductionLabel(stage?: string | null) {
  if (!stage) return "—";
  if (stage === "for_print") return "For Print";
  if (stage === "printed" || stage === "print_qc") return "Printed";
  if (stage === "framed" || stage === "frame_qc" || stage === "final_check") return "Finishing";
  ...
  const visibleOrders = useMemo(() => {
    ...
    return data.orders.filter((order) => {
      ...
      const members = data.orderMembers
        ...
        .filter(Boolean);
      const memberText = members.map((member) => `${member!.full_name} ${member!.organization ?? ""} ${member!.batch ?? ""} ${member!.contact_number ?? ""}`).join(" ");
      const matches = !needle || `${order.order_number} ${person?.full_name ?? ""} ${person?.organization ?? ""} ${person?.batch ?? ""} ${person?.contact_number ?? ""} ${memberText}`.toLowerCase().includes(needle);
    ...
    return data.payments.filter((payment) => {
      ...
      const members = order
        ? data.orderMembers
            ...
            .filter(Boolean)
        : [];
      const memberText = members.map((member) => `${member!.full_name} ${member!.organization ?? ""}`).join(" ");
      return !needle || `${order?.order_number ?? ""} ${person?.full_name ?? ""} ${person?.organization ?? ""} ${memberText} ${payment.reference ?? ""} ${payment.method}`.toLowerCase().includes(needle);
    });
  }, [data, query]);
    ...
    <AppShell>
      ...
      {!data.event ? <EmptyState title="No event selected" description="Choose an event before managing orders." action={<Button asChild><Link to="/events">Open Events</Link></Button>} /> : (
        <>
          ...
          {view === "orders" ? <><div className="mb-4 flex gap-1 overflow-x-auto">{(["all", "unpaid", "partial", "paid", "cancelled"] as const).map((value) => <Button key={value} size="sm" variant={filter === value ? "default" : "ghost"} onClick={() => setFilter(value)}>{titleize(value)}</Button>)}</div>{visibleOrders.length ? <div className="grid gap-3">{visibleOrders.map((order) => { const person = findParticipant(data, order.participant_id); const items = data.orderItems.filter((item) => item.order_id === order.id); const members = data.orderMembers.filter((member) => member.order_id === order.id).map((member) => findParticipant(data, member.participant_id)).filter(Boolean); const displayPeople = members.length ? members : [person]; const memberNames = members.map((member) => member!.full_name); return <Panel key={order.id} title={person?.full_name ?? order.order_number} description={`${order.order_number} · ${memberNames.length > 1 ? `${memberNames.length} people · ${memberNames.join(", ")}` : person?.organization ?? "No congregation"} · ${person?.batch ?? "No class/group"}`} actions={<div className="flex gap-2"><StatusPill label={order.payment_status} tone={paymentTone(order.payment_status)} /><StatusPill label={simpleProductionLabel(order.production_status)} tone={order.status === "cancelled" ? "neutral" : productionTone(order.production_status)} /></div>}><div className="grid gap-3">
  <div className="flex flex-col gap-3 rounded-lg border border-border bg-muted/10 p-3 lg:flex-row lg:items-center">
    <div className="flex min-w-0 items-center gap-3 lg:flex-1">
      <PeopleAvatars people={displayPeople} size="md" />
      <div className="min-w-0">
        <p className="truncate text-sm font-semibold">{memberNames.length > 1 ? memberNames.join(" · ") : person?.full_name ?? "Participant"}</p>
        <p className="mt-0.5 truncate text-xs text-muted-foreground">{person?.organization ?? "No congregation"} · {person?.contact_number ?? "No contact"}</p>
      </div>
    </div>
    <div className="grid grid-cols-3 gap-4 text-sm lg:min-w-[330px]">
      <div><p className="text-[0.65rem] uppercase tracking-[.08em] text-muted-foreground">Total</p><p className="mt-0.5 font-semibold">{peso(order.total)}</p></div>
      <div><p className="text-[0.65rem] uppercase tracking-[.08em] text-muted-foreground">Paid</p><p className="mt-0.5 font-semibold">{peso(order.paid)}</p></div>
      <div><p className="text-[0.65rem] uppercase tracking-[.08em] text-muted-foreground">Balance</p><p className={cn("mt-0.5 font-semibold", orderBalance(order) > 0 && "text-warning")}>{peso(orderBalance(order))}</p></div>
    </div>
  </div>
  <div className="flex flex-wrap gap-2">{items.map((item) => <span key={item.id} className="rounded-md border border-border bg-muted/20 px-2.5 py-1.5 text-xs"><strong>{item.kind === "group_package" ? "Class" : "Solo"}</strong> · {item.quantity}× {item.print_size}{item.framed ? ` · ${item.frame_color ? item.frame_color[0].toUpperCase() + item.frame_color.slice(1) : "Black"} frame` : ""}</span>)}</div>
  {order.status !== "cancelled" ? <div className="flex justify-end"><Button size="sm" variant="ghost" onClick={() => void cancelOrder(order.id, order.order_number)}><XCircle className="size-4" /> Cancel order</Button></div> : null}
</div></Panel>; })}</div> : <EmptyState title="No matching orders" description="Orders created from the client gallery appear here automatically." />}</> : visiblePayments.length ? <div className="grid gap-3">{visiblePayments.map((payment) => { const order = data.orders.find((item) => item.id === payment.order_id); const person = order && findParticipant(data, order.participant_id); return <Panel key={payment.id} title={person?.full_name ?? "Payment"} description={`${order?.order_number ?? "Unknown order"} · ${titleize(payment.method)} · ${payment.reference ?? "No reference"}`}><div className="flex flex-col gap-3 sm:flex-row sm:items-center"><div className="min-w-0 flex-1"><p className="font-display text-2xl font-extrabold">{peso(payment.amount)}</p><p className="mt-1 text-xs text-muted-foreground">{person?.batch ?? "No class/group"}</p><div className="mt-2 flex flex-wrap gap-2"><StatusPill label={payment.status} tone={payment.status === "verified" ? "success" : "warning"} />{order ? <StatusPill label={`Order ${order.payment_status}`} tone={paymentTone(order.payment_status)} /> : null}</div></div><div className="flex flex-wrap gap-2">{payment.proof_url ? <Button size="sm" variant="outline" onClick={() => void openProof(payment.proof_url!)}><ExternalLink className="size-4" /> View screenshot</Button> : null}{payment.status !== "verified" ? <Button size="sm" onClick={() => void verifyPayment(payment.id)}><CheckCircle2 className="size-4" /> Verify payment</Button> : null}<Button size="sm" variant="ghost" onClick={() => void removePayment(payment.id)}><Trash2 className="size-4" /> Remove</Button></div></div></Panel>; })}</div> : <EmptyState title="No payment records" description="Client payment submissions and staff-recorded payments appear here." />}
        </>
      )}
    </AppShell>
  );
